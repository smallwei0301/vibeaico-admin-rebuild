...
export const tripsPage = {
  ...
  plans: {
    ...
    quick: {
      ...
      previewLink: '開啟商店頁預覽',
      advancedHint: '需要調整人數、訂金、時長或季節定價時，開啟進階設定；成團門檻與販售方式等會在後續切片提供。',
      save: '儲存快速編輯',
      saving: '儲存並確認中…',
    },
    advanced: {
      title: '方案進階設定',
      intro: '這一版提供時長、計價方式、人數、訂金政策與季節定價；成團門檻與販售方式（固定團次／自選時間／先申請再確認）等會在後續切片提供。',
      open: '開啟進階設定',
      backToQuick: '回到快速編輯',
  ...
  confirm: {
    ...
    deleteSeason: (name: string) => `確定要刪除季節「${name}」嗎？`,
