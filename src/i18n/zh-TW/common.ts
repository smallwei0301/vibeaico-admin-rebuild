...
export const common = {
  ...
  bugReport: {
    ...
    contactEmail: '聯絡信箱',
    contactEmailHint: '選填；會隨這次回報內容一併保存。',
    submit: '送出回報',
   ...
   * 右下角小幫手。
   *
   * ⚠️ 這段文案原本是「我是 VibeAI 平台的 AI 客服助理，可以幫您查 LINE 狀態、
   * 推播額度、最近異常日誌，或回答後台使用問題」，而元件送出後什麼都不做。
   * 現在它真的會查前兩項（外加「方案與權益到期」），所以：
   *
   * - 不再稱「AI」：判定是一張寫死的關鍵字表，把規則比對包裝成 AI 是另一種假成功。
   * - 不再宣稱「最近異常日誌」：這個系統沒有任何錯誤日誌表。
   * - 不再宣稱「回答後台使用問題」：那需要真的語言模型，這一版沒有。
   */
  supportChat: {
    title: '後台小幫手',
    greeting:
      '您好，我是後台小幫手。我不是 AI，是一組寫死的查詢，只查得到三件事：LINE 串接狀態、本月推播額度、方案與權益到期。其他問題我不會猜，會直接說我看不懂。',
    placeholder: '例如：LINE 串接好了嗎？本月還能推播幾則？',
    send: '送出',
    sending: '查詢中…',
    failed: '查詢失敗，請稍後再試一次。',
    suggestionsTitle: '可以問我：',
