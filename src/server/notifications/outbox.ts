  ...
  if (outbox.event_name === 'PLATFORM_NOTIFICATION_HEALTH' || outbox.event_name === 'PLATFORM_NOTIFICATION_ALERT') {
    ...
    if (delivery.channel === 'EMAIL') {
      ...
      return sendEmailWithResend({
        ...
        subject: message.subject, html: message.html, idempotencyKey: `notification-delivery:${delivery.id}`,
      });
    }
...
async function persistOutcome(admin: Admin, delivery: ClaimedDelivery, outcome: TransportOutcome): Promise<boolean> {
  const transition = deliveryTransition(outcome, new Date(), delivery.attempt_count);
  const { data: updatedDelivery, error } = await admin.from('notification_deliveries').update({
  ...
  if (transition.bindingInvalid) {
    const { error: bindingError } = await admin.from('telegram_bindings').update({
      active: false, invalid_reason: transition.lastErrorCode, invalidated_at: new Date().toISOString(),
    })
