...
export type BookingPolicyDecision =
  ...
  | {
      allowed: true;
      checkoutMode: TripBookingType;
      deposit: DepositRule;
    };

/**
 * Resolves the tenant-private traveler override at the booking boundary.
 * Hard operational and payment gates always win over a traveler override.
 */
export function resolveTravelerBookingPolicy(input: PolicyInput): BookingPolicyDecision {
  ...
  if (input.policy.kind === 'FORCE_DEPOSIT') {
    ...
    return {
      allowed: true,
      checkoutMode: input.plan.salesMode,
      deposit: { mode, value },
    };
  }
  ...
  return {
    allowed: true,
    checkoutMode: input.policy.kind === 'REQUEST_ONLY' ? 'REQUEST' : input.plan.salesMode,
