 ...
 * 因此：
 *
 *   顧客打「選單」→ resolveBuiltinIntent → 'MENU' → replyFlexMenu()
 *                                                  └→ buildFlexMenuOutcome()（本檔）
 *
 * ⚠️ 誠實標註 —— issue #6 還有一條**沒有在本輪落地**的要求：
 * 「Rich Menu 格子若設 FLEX_POPUP 也走同一組組裝函式」。
 * 那一條的前提是「Rich Menu 每格可以各自設定 action」，而 main 上
 * `POST /api/settings/line/rich-menu/create` 的六格 action 是寫死的
 * `CELL_TEXTS`，**沒有任何每格設定的儲存後端**（屬 issue #7 / #19）。
 * 先把 `richMenuCellAction()` 搬過來只會多一支沒有任何呼叫端的函式，
 * 卻讓讀到的人以為每格設定已經會生效。等每格設定真的存得下來時，
 * 那一格送出的文字要是 `resolveBuiltinIntent()` 解得到 `MENU` 的字
 * （主選單／選單／功能），才會走到本檔——單一事實來源這條規則不變。
 */
import {
