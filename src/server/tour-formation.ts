...
export type FormationDecision = {
  ...
  acceptedRiskParticipants?: number;
};
...
export type FormationTransitionInput = {
  ...
  minToDepart: number;
  /** 導遊已接受風險時的人數；同一人數不重複觸發 AT_RISK。 */
  acceptedRiskParticipants?: number | null;
...
export function qualifiesForFormation(input: FormationEligibilityInput): boolean {
  ...
  if (!Number.isFinite(input.paidAmount) || !Number.isFinite(input.upfrontRequiredAmount)
      || !Number.isFinite(input.totalAmount) || input.paidAmount < 0
      || input.upfrontRequiredAmount < 0 || input.totalAmount < 0
      || input.paidAmount > input.totalAmount || input.upfrontRequiredAmount > input.totalAmount) {
    return false;
  }
  ...
  switch (input.depositMode) {
    ...
    case 'DEPOSIT_PERCENT':
      return (input.paymentStatus === 'PARTIAL' || input.paymentStatus === 'PAID')
        && input.paidAmount >= input.upfrontRequiredAmount
        && (input.paymentStatus !== 'PARTIAL' || input.paidAmount > 0);
    case 'FULL':
      return input.paymentStatus === 'PAID' && input.paidAmount >= input.totalAmount;
  }
}
...
export function transitionFormation(
  ...
  if (input.trigger === 'QUALIFYING_CANCELLATION') {
    if (current === 'FORMED' && !hasMinimum
        && (input.acceptedRiskParticipants == null
          || input.qualifyingParticipants < input.acceptedRiskParticipants)) {
  ...
  if (input.trigger === 'GUIDE_OVERRIDE_FORM' && current === 'REVIEW_REQUIRED') {
    return {
      status: 'FORMED', formedBy: 'GUIDE_OVERRIDE',
      acceptedRiskParticipants: input.qualifyingParticipants,
    };
  }
  ...
  if (input.trigger === 'GUIDE_CONTINUE' && current === 'AT_RISK') {
    return { status: 'FORMED', acceptedRiskParticipants: input.qualifyingParticipants };
  }
