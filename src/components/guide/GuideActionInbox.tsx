...
import type { GuideActionInbox as Inbox, GuideActionItem } from '@/lib/types';
...
function ActionCard({ item, timeZone }: { item: GuideActionItem; timeZone?: string }) {
  return (
    <li className="flex min-w-0 flex-col gap-3 rounded-lg border border-border p-3 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1">
        ...
        {item.detail ? <p className="truncate text-xs text-secondary">{item.detail}</p> : null}
        {item.dueAt ? (
          <p className="mt-1 text-xs text-secondary">
            {t.guideInbox.deadline(new Intl.DateTimeFormat('zh-TW', {
              dateStyle: 'short', timeStyle: 'short', timeZone: timeZone || 'Asia/Taipei',
            }).format(new Date(item.dueAt)))}
            {item.overdue ? <Badge className="ml-2" tone="danger">{t.guideInbox.overdue}</Badge> : null}
          </p>
        ) : item.createdAt ? <p className="mt-1 text-xs text-secondary">{t.guideInbox.receivedAt(new Intl.DateTimeFormat('zh-TW', {
          dateStyle: 'short', timeStyle: 'short', timeZone: timeZone || 'Asia/Taipei',
        }).format(new Date(item.createdAt)))}</p> : null}
      </div>
      <Link href={item.href} className="btn btn-primary btn-sm w-full justify-center sm:w-auto sm:flex-shrink-0">
        {t.guideInbox.actions[item.reason]} <ChevronRight size={14} />
      </Link>
    </li>
  );
}
...
export function GuideActionInbox() {
  ...
  React.useEffect(() => {
    let active = true;
    void getGuideActionInbox().then((value) => { if (active) setInbox(value); })
  ...
  return (
    <Card className="mb-4">
      <CardHeader><CardTitle><AlertCircle size={18} className="text-warning" />{t.guideInbox.title}</CardTitle></CardHeader>
      <CardBody>
        {!inbox ? <p className="text-sm text-secondary" aria-live="polite">{t.guideInbox.loading}</p>
          : SECTIONS.every(({ key }) => inbox[key].length === 0) ? (
            ...
            <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
              {SECTIONS.map(({ key, title, tone }) => (
                <section key={key} className="min-w-0" aria-labelledby={`guide-inbox-${key}`}>
                  <h3 id={`guide-inbox-${key}`} className="mb-2 flex items-center gap-2 text-sm font-bold">
                    <CalendarClock size={15} />{title}<Badge tone={tone}>{inbox[key].length}</Badge>
                  </h3>
                  {inbox[key].length ? <ul className="space-y-2">{inbox[key].map((item) => <ActionCard key={item.id} item={item} timeZone={inbox.timeZone} />)}</ul>
                    : <p className="rounded-lg bg-surface-subtle p-3 text-sm text-secondary">{t.guideInbox.emptySection}</p>}
                </section>
              ))}
            </div>
          )}
