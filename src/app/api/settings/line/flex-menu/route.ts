 ...
 * 就是一個沒人看得見的假成功。
 *
 * `linkUrl`：卡片的 optional 連結網址。可用 scheme 是**白名單**，唯一出處為
 * src/config/tenant-settings.ts 的 `isAllowedFlexLinkUrl()`（本端點 zod、
 * webhook 讀取路徑、頁面前端三處共用）。黑名單只擋得住今天想得到的字串，
 * 明天多一個沒人想過的 scheme 就會直接送到顧客手上，而沒有任何測試會紅。
 */
