...
`MODEL_GOVERNANCE` is only for pure model/Agent/governance work: model routing, orchestration, WIP/Final Risk guard itself, governance metrics/scoreboard, PR lifecycle, governance CI/templates and their tests/docs. Use the currently available model: no pinned executor, no Product Terra/Reserve lane, no Astra/Fable Final Risk. See Owner decision #360, `docs/decisions/2026-09-11-owner-governance-unpinned-model.md`.
...
Model values above are truthful fallback examples, not proof of an execution. Record concrete values only with a reliable source. Unknown does not block pure governance, and does not become verified evidence.
...
<!-- Keep exact FIELD: value lines. The WIP Guard parses them. PRODUCT_MAINLINE uses B+ lanes; MODEL_GOVERNANCE uses GOVERNANCE with no specified executor model. -->
...
- REQUESTED_MODEL / ACTUAL_MODEL: <!-- Record actual values; governance may use requested=not_requested; actual=unknown -->
...
- Review verdict (Product: Sol; governance: current executor):
