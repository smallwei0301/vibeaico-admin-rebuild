...
description: "Use for long-running /goal, continued autonomous delivery, model switches, open-Issue reduction, multi-agent delegation, WIP control, shared TEST scheduling, CI classification, high-risk review, PR cleanup, scorecard generation, or Issue closeout in smallwei0301/vibeaico-admin-rebuild. Enforces B+: one MAIN Terra, one source-only RESERVE Terra, one Luna Closure lane, 3-6 narrow Luna tasks, one shared TEST holder, at most two active candidates, and a measurable report loop."
metadata:
  author: smallwei0301
  version: "0.5.0"
---
...
This skill is an execution adapter. Canonical policy order:

1. `origin/main:docs/decisions/2026-09-01-owner-bplus-delivery-loop.md`
2. `origin/main:docs/AGENT-EXECUTION.md`
3. `origin/main:docs/AGENT-BPLUS-DELIVERY-LOOP.md`
4. `origin/main:docs/PR-LIFECYCLE.md`

Older Mode C decisions are historical wherever they allow multiple complete Terra BUILD lanes.

## Trigger

Use this skill for `/goal`, “繼續施工”, multi-Agent routing, CI／TEST coordination, Closure Sweep,
WIP review, scorecard creation, or model-usage optimization.
...
3. Read live open Issues／PRs, exact heads, CI, TEST holder and latest 1–3 run reports.
4. Create or resume `RUN_ID` and its JSON ledger before opening new work.
...
`RESERVE_TERRA` is not a second delivery line. It may produce at most one atomic source-only commit,
then stops at `READY_FOR_PROMOTION`.
...
LUNA_AGGREGATOR deduplicate all Luna results into <=30 lines
...
LUNA_CLOSEOUT   evidence, status, Issue close, lane release, run report
```
...
Do not copy full chat history or make multiple Luna agents scan the same open-Issue list. One
Aggregator removes duplicates before Sol sees the result.
...
RESERVE starts only while MAIN is genuinely waiting and has no source work. It must set:
...
A lane transition dispatch must match exact PR, branch and SHA. No no-op commits and no unchanged
reruns.
...
Record actual token data when available. Otherwise keep it `null` and use internal requested-model
weights Luna=1, Terra=3, Sol=6 with compact=1, medium=1.5, full=3. The report must say the weights are
not OpenAI's official usage conversion and identify any `actualModel=unknown` task.
...
- Two runs without CLOSED or complete OWNER_BLOCKED: enter `CLOSURE_RECOVERY` and open no new large Issue.
...
Luna or the main Agent performs the mechanical close only after `CLOSE_APPROVED`.

## Continue rule

A progress update, CI wait, TEST wait, model switch, commit, PR, one completed Issue or one blocked
Issue is not a global stop. Continue the B+ loop until canonical stop conditions are met, while keeping
only one complete MAIN Terra line.
