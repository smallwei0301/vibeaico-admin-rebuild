...
1. `origin/main:docs/decisions/2026-09-11-owner-model-governance-model-agnostic.md`
2. `origin/main:docs/decisions/2026-09-07-owner-governance-alignment.md`
3. `origin/main:docs/decisions/2026-09-01-owner-bplus-delivery-loop.md`
4. `origin/main:docs/decisions/2026-09-01-owner-natural-loop-commands-and-completion-truth.md`
5. `origin/main:docs/AGENT-EXECUTION.md`
6. `origin/main:docs/AGENT-BPLUS-DELIVERY-LOOP.md`
7. `origin/main:docs/AGENT-PROJECT-COMMANDS-AND-TRUTH.md`
8. `origin/main:docs/PR-LIFECYCLE.md`
9. `origin/main:scripts/agents/model-routing.json`

The 2026-09-11 Owner decision wins on model-governance execution: MODEL_GOVERNANCE is model-agnostic. Do not select, score or audit it by requested/actual model. Product B+ topology and Product model/risk routing still apply to PRODUCT_MAINLINE.
...
Use for model routing policy, Agent orchestration, WIP / Final Risk guard behavior, governance metrics/scoreboards, PR lifecycle, governance-only CI policy, governance templates and related durable policy docs.
...
This topology is for `PRODUCT_MAINLINE`. Do not create Product Terra slots for `MODEL_GOVERNANCE`.

## Router

```text
MODEL_GOVERNANCE
  GOVERNANCE         current truth -> bounded implementation -> tests -> final diff verification -> closeout
...
For `MODEL_GOVERNANCE`, do not perform model dispatch merely because governance files changed. Do not load or dispatch the Astra/Fable Product Final Risk path for pure governance. Source CI/tests and a final exact-diff read are still mandatory.
...
For MODEL_GOVERNANCE, use one bounded implementation, required tests, then re-read the final exact diff and current main before merge. Do not create ceremonial duplicate model reviews without new evidence.
...
MODEL_GOVERNANCE is excluded from model-utilization analysis. Do not count governance work as Terra/Sol/Astra/Fable utilization and do not infer a model from its lane.
...
For PRODUCT_MAINLINE, record actual token/model data when available and keep unavailable values null/unknown. For MODEL_GOVERNANCE, do not analyze which model was used; model identity is outside its retrospective/scoreboard quality criteria.
...
MODEL_GOVERNANCE does not spawn Product Terra merely to improve utilization metrics.
...
For MODEL_GOVERNANCE, the active governance Agent/session may perform mechanical close after required tests, final exact-diff verification and Completion Truth. No named model is required for that closeout.
