...
永久體驗方案以「**累積有效訂單數**」作主要升級門檻：

- 在累積上限尚未用完前，可持續接受新的有效預約，不因註冊時間經過而失效。
- 達到累積上限後，停止建立會超過免費額度的**新有效訂單**；不把整間後台鎖死。
- 已成立／付款中／待退款／已成團的既有訂單仍可完整處理。
- 既有付款、退款、通知、安全、資料查看與匯出仍可使用。
- 升級 SaaS 後立即恢復接受新預約，不搬移資料、不要求重建租戶。
- 系統應在接近上限時提前顯示白話提醒，不等旅客送出最後一步才突然失敗。
...
**累積上限的確切數字仍待 Owner 下一題拍板。** 在數字未定前，可先完成 entitlement（權益）資料模型、用量計數介面與邊界測試，但不得自行寫死 10／20／50 等數字。
...
在價格／上限未拍板前，可以完成 capability matrix（能力矩陣）、entitlement evaluator、GUIDE UI 原型、累積用量計數與相容測試；不得啟動 Production 真實訂閱扣款。