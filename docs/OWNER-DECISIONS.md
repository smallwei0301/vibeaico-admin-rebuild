...
| #120 / #48 | GUIDE 團隊超額導遊席次 | **團隊版 NT$799／月含 5 位 active+bookable 導遊；第 6 位起每增加 1 位，加 NT$150／月。** | 只計目前 active+bookable 人員，停用／歷史導遊不占付費席次；不建立另一套大團隊模式。年繳超額席次是否同樣採付 10 個月送 2 個月仍待下一題裁示。canonical：`docs/decisions/2026-09-03-guide-saas-pricing.md`。 |

## 2026-09-08 已裁示

| Issue／PR | 主題 | Owner 決策 | 後續實作重點 |
|---|---|---|---|
| #209 / #290 | 最後風險評估模型 allowlist | **保留 `claude-fable-5-1` 作為 `models.finalRisk` 預設模型，並以 `models.finalRiskModelCatalog`／`models.finalRiskAllowedModels` 明確限制目前可用的 GPT-6 Astra（`gpt-6-astra`）或 Fable（`claude-fable-5-1`）。`requestedModel` 與 `actualModel` 必須是同一個允許模型；trusted review、exact head、operator attestation 與報告 URL 不變。** | 缺失、非陣列、空值、重複或 catalog 外模型一律 fail closed；政策版本升為 `2026-09-08.3` 以使既有 attestation 重新驗證。canonical：`docs/decisions/2026-09-08-owner-final-risk-allowlist.md`。 |

## 2026-09-07 已裁示

| Issue | 主題 | Owner 決策 | 後續實作重點 |
|---|---|---|---|
| #50 / #8 / #259 / #27 / #31 | 正式庫 DDL（0086／0087）＋ 指向 TEST 的 Preview ＋ #259 加欄位 | **① 授權將 `0086_keyword_reply_images_bucket.sql` 與 `0087_issue_8b_tour_orders.sql` 套用到正式庫 `egehnijjpgijmccagxac`（逐次具名，只涵蓋這兩支）。② 授權為 Vercel Preview 環境設定指向 TEST 專案 `nmwhwngojosmagjuvxol` 的變數並以 `preview/**` 分支部署，供 #27／#50／#31 的瀏覽器實測。③ 授權為 #259 的五個欄位寫一支只 add column 的 migration 並套用。** | 前兩支 migration 都是「程式碼已上線、schema 沒跟上」，且都由本輪合併造成：`0086` 讓正式站「附加圖片」不再 `500 SYS_001`；`0087` 讓旅遊訂單鏈路在正式站真的通。`0086` 已於 2026-09-07 套用並留下套用前後唯讀證據；`0087` 待 #271 合併後套用（source 必須來自已合併的 `main`，#197）。Preview 只動 Preview 環境變數，不動 Production 設定。不含真實顧客通知、金流、退款、手動 Production 部署。canonical：`docs/decisions/2026-09-07-owner-production-ddl-0086-0087-0089.md`。 |
...
| Issue | 主題 | Owner 決策 | 後續實作重點 |
|---|---|---|---|
| #120 / #48 | GUIDE SaaS 正式價格骨架 | **永久體驗版 NT$0，1 位導遊、累積 30 張有效訂單；個人版 NT$399／月，1 位導遊；團隊版 NT$799／月，含 5 位 active+bookable 導遊。** | 第 30 張免費有效訂單可成立，第 31 張起要求升級；既有訂單／退款／通知／資料仍可處理。團隊 5 席包含 owner；停用與歷史人員不占新席次。AI／LINE 加購及 Production subscription billing 仍待後續裁示；年繳折扣已於 2026-09-09 裁示為付 10 個月送 2 個月，超過 5 位後每席 +NT$150／月。canonical：`docs/decisions/2026-09-03-guide-saas-pricing.md`。 |
