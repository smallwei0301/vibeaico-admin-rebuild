...
| GET/POST `/api/bookings/:id/addons`、DELETE `/api/bookings/:id/addons/:addonId` | 預約加購明細；寫入只能經 `0053` 的原子 RPC，見 B-1.1。 |
...
`notify=true` 走既有 LINE 憑證 → quota → push receipt；quota reservation 是 `0054` 的單一
conditional upsert，資料庫/feature 查詢失敗時 fail closed、不送 LINE；結果一律記為
`NONE|LINE|NO_LINE|NOT_CONFIGURED|QUOTA_EXCEEDED|FAILED`；結果標記 RPC 僅供 route 的
service-role client 呼叫，且由 route 先完成 MANAGER 租戶授權。`performance_staff_id`
...
此 committed conflict 讓 UI 關閉 add-on/detail target、遞增 revision 後重新載入，不能重送同一筆。
其他非 LINE 結果也照實回傳。前端 mock 分支明確是合成資料。
`0054` 的 add/delete RPC execute 僅給 `authenticated`，保留其中的 MANAGER/tenant
檢查；service role 只執行 server-internal notification marker 與 quota reservation，並保有
