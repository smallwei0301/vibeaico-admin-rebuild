...
**上下文必須同時涵蓋服務與行程**（導遊型租戶只有行程、沙龍只有服務、斜槓兩者都有）。
context 一律由 11 分冊的 `catalog` 端點同一組查詢組出來，**不得另外實作一份查詢**，
確保 AI 講的與商店頁、LINE 輪播完全一致。
...
export type ShopContext = {
  ...
  trips: string[];
  /** 未來 14 天可售團次與**即時剩餘名額**：「龜山島賞鯨 標準團 8/23(六) 09:00 剩 3 位」 */
  departures: string[];
...
- **走 reply 不走 push**（不佔推播額度）。但 LINE 的 `replyToken` 有時效，
  因此 AI 呼叫**必須設 10 秒逾時**（`AbortSignal.timeout(10_000)`）；
  逾時或失敗 → catch 後回 null（落回 `defaultReply`），**不重試**。
- AI 回覆同樣寫入 `chat_messages`（direction='OUT'，message_type='ai'），
  導遊在後台聊天頁看得到 AI 幫他回了什麼。
- 團次資料每次呼叫都即時查（不快取）—— 名額是會變的。
