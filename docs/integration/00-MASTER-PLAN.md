...
8. **每完成一個 Phase 都要過驗收**：`npm run typecheck` 零錯誤、`npm run build` 成功、
   ...
   **打勾必須附證據**（08 分冊開頭的打勾規則）：每一勾都要註明驗證方式——測試檔名:案例名，
   或人工實測的日期＋步驟＋結果。無法舉證的項目不得打勾。這條是 2026-08-24 稽核
   （14 分冊）加嚴的：先前「rich menu 建立/發布」在只有 API 測試、頁面按鈕從未呼叫
   後端的狀態下被打勾，使用者按了「發布」看到成功、LINE 端卻什麼都沒發生。
...
12. **禁止假成功（UI 版的「不要製造假的已知」）。** 任何顯示成功訊息（toast、
    狀態變綠、欄位清空）的使用者操作，其副作用必須真的發生在 DB 或外部系統；
    只改本地 React state 就報成功、用 `setTimeout` 假延遲冒充網路請求，一律視同
    違反鐵則。骨架期允許的本地模擬，一旦該功能的後端存在就必須接上，或把成功
    訊息改為誠實的「尚未建置」。API 端點「存在且整合測試綠」**不等於**功能完成——
    頁面 handler → `src/services/*` → 端點這條鏈路完整才算（12 §6 第 10、11 條）。
