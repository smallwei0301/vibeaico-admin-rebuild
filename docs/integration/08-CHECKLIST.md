...
> ## ⚠️ 2026-08-26：本檔有數處「重開」註記已經過期，但**刻意沒有代為打勾**
>
> 一輪 issue 驗收稽核（涵蓋全部 34 個 issue）發現：本檔的 Phase 2 / Phase 6 有幾列
> 仍帶著 2026-08-24 的「重開」註記與理由，而**那些理由描述的缺陷已經被後續的
> issue 修好了**（例如 issue #4／#5／#15 關閉時已處理掉對應項目）。
>
> **為什麼不直接打勾**：本檔第 1 條規則就是「沒有證據不准打勾」。要打這些勾，
> 得逐列去驗「那個重開理由現在還成不成立」——那是一輪需要跑測試與讀程式碼的
> 工作，不是文件整理。稽核者的權限只到「指出落差」，代為打勾就變成他自己在
> 製造一個沒有證據的勾。
>
> **已知過期的列**（行號以 2026-08-26 的版本為準，接手人請以內容比對而非行號）：
> `:47` 登出、`:70` block-times／available-slots、`:98` 每頁實測紀錄、
> `:115` keyword-replies 管理頁、`:120` line-notify 額度、`:132` 假發布、`:142` verify 端點。
>
> `:83`（B-6）與 `:229`（upload 死按鈕）**已經在 2026-08-26 更新過**，是正確的樣子——
> 可以拿它們當格式範本：**保留原本的重開理由，在下面加一行「重開理由已消除，
> 依據是什麼」，而不是把重開理由刪掉。**
>
> 相關 issue 的留言裡有各自的證據，接手人可以從那裡開始。
