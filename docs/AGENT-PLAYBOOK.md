...
### PB-014 — schema-qualified PostgreSQL types must use catalog names

- 首次／最近：2026-08-31／2026-08-31
- 發生次數：1
- Issue／PR／CI：Issue #41／PR #73；TEST migration `0046_issue_41_formation_transition_revision.sql`
- 分類：Migration／TEST DB
- 事件：未套用的 0046 以 `pg_catalog.bigint` 宣告 transition revision，首次 TEST 套用在 `ALTER TABLE` 失敗。
- 證據：PostgreSQL `42704`：`type pg_catalog.bigint does not exist`；TEST baseline 已有 #41 的 0040／0041，0046 未寫入 migration history。
- 根因：把 SQL 的可讀 alias 當成 `pg_catalog` 下可 schema-qualify 的型別名；catalog 對應名稱是 `int8`。
- 影響：#41 的 recurring lifecycle-event idempotency repair 尚未在 TEST 驗證，#42／#12 仍不能以它為已解除依賴。
- 修正：只修未安裝的 forward migration 0046，將 column 與 PL/pgSQL variable 改為 `pg_catalog.int8`；0040／0041 檔案保持不改。因 TEST history 記錄的 0041 仍是舊 direct-outbox 函式，先以目前 0041 hardening source 做一次 idempotent function repair，再套 0046，避免 0041 覆蓋 epoch-aware bridge。
