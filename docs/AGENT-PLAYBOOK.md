...
### PB-003 — seed 把缺欄位錯誤判成 optional table
...
- 證據：
  ...
  select column_name, is_nullable, column_default from information_schema.columns
   where table_name='public' and column_name='deposit_mode_snapshot';
...
- 發生次數：本延伸 1 次；原條目四種形態保留，不重算來源中的历史數字。
...
- 修正：本轮不改三本 Run、不補零值與歷史用量；已把分離判讀寫進速查及下表。
...
2. **先列可證明的事，再列待證明的事。** 同一個結果分開写「本輪實測、來源記載、
