...
> 若兩者衝突，以 `docs/AGENT-EXECUTION.md` 為準。
...
2. 新舊 PR 對應同一主要 Issue；
3. 舊 PR 仍是 open；
4. GitHub commit compare 證明新 head 包含舊 head，compare status 為 `ahead` 或 `identical`；
5. 沒有 API / 權限 / compare 不確定性。
