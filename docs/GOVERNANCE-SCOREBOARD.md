...
因此 Governance Scoreboard v2 的核心問題是：
...
自 `docs/metrics/governance-scoreboard-policy.json` 的 v2 `effectiveAt` 起，新 terminal Governance Run 使用 contract v2。
...
`NOT_APPLICABLE` 是非模型 sentinel（不適用標記），不得被當成模型名稱、model execution evidence 或 Scoreboard coverage。
...
```json
{
  ...
  "records": [
    {
      "id": "github:pr#363/review#final",
      "subject": "pr#363",
      "role": "GOVERNANCE_REVIEW",
      "executionRef": "github:pr#363/review#final",
...
```json
{
  "reconciliation": {
    ...
    "byRecordId": "github:pr#363/review#final-pass"
  }
}
```
...
1. core metric data quality >= policy 門檻，現行為 95%；
...
2026-09-11 v2 生效前的 Governance Scoreboard v1 與 review evidence 全部維持 read-only history。
