...
| 真實租戶／店家設定變更（含其外部服務設定，如 LINE webhook endpoint） | 允許 | 見 §3.2；由高風險審查制度治理，不需逐次具名授權 |
| Production DDL／DML／migration／reset／seed | **禁止** | 需針對精確專案與範圍的新授權 |
| Production deployment／promote／流量切換 | **禁止** | 需新授權 |
| 真實付款、退款、訂單，或主動發送真實顧客通知 | **禁止** | 測試只用 sandbox、mock 或明確安全接收者 |
| 輸出或提交 token、密碼、key、完整 `.env` | **禁止** | 秘密只在執行環境短暫使用 |

### 3.2 真實租戶設定變更（Owner 2026-09-11 裁示）

真實租戶／店家的設定變更不再需要逐次具名授權，改由既有高風險審查制度治理。
完整裁示見 `docs/decisions/2026-09-11-owner-tenant-config-under-high-risk-review.md`。

下列條件必須**全部**成立，任一項不成立即退回停泊並回報：

1. **先查現況**。變更前唯讀確認目標的當下狀態，並把結果寫進證據。
   「Issue 這樣寫」不是現況證據——#251 的端點在授權當天實查已是正確值，照 Issue
   內文動手會變更一個本來就正確的設定。
2. **可逆，且先記錄舊值**。不可逆的設定變更不適用本節，仍需逐次具名授權。
3. **變更後以獨立查詢驗證**。API 回 200 不是完成證據；必須重讀該設定，並在可行時
   以無副作用探測確認行為。
4. **依既有高風險分類路由**。改變外部系統打進來的路徑、跨越租戶邊界或影響既有 gate 的
   設定變更，依 `scripts/agents/model-routing.json` 的 `highRisk` 分類走 Final Risk。
5. **祕密欄位除外**。不得在同一動作中新增、變更或輪替憑證。讀取並使用既有憑證以完成
   設定變更是允許的；輸出憑證內容一律禁止。

本節放寬的是「誰批准」，不是「要不要查證」。條件 1 與 3 把查證從口頭要求變成
可稽核的前後置步驟。
