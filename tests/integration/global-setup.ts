...
function spawnNextDevServer(): ChildProcess {
  ...
  const child = spawn(resolve(REPO_ROOT, 'node_modules/.bin/next'), ['dev', '-p', String(PORT)], {
    ...
    env: {
      ...
      // Explicitly enable the bounded, test-only webhook drain seam. The route
      // still requires its dedicated request header and always rejects it in production.
      LINE_WEBHOOK_DRAIN_ENABLED: 'true',
