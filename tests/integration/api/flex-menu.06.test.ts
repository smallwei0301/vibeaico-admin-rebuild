...
describe('端到端：API 存卡片 → 顧客打「選單」→ mock LINE 收到 Flex（bubble 數＝卡片數）', () => {
  ...
  /*
   * ⚠️ issue #6 的「Rich Menu 格子設 FLEX_POPUP 也走同一條路徑」**本輪沒有落地**，
   * 所以這裡沒有對應的斷言。前提還不成立：main 的 rich-menu/create 六格 action
   * 寫死在 `CELL_TEXTS`，沒有任何「每格自訂」的儲存後端（屬 #7 / #19）。
   * 那一段落地時，這一條要補回來——上面的同義詞案例已經證明「選單」這個字
   * 本身走得到 Flex，屆時只要再證那一格真的送出這個字。
   */
