 ...
 *      也只有它會組 `uri` action。第二份組裝短期看起來一樣，長期一定分岔，
 *      而分岔的那一天沒有任何測試會紅。
...
/* ========================================================================== */
/* 結構要求 ①：Flex JSON 的組裝只有一個出處                                    */
/* ========================================================================== */
describe('Flex JSON 的單一事實來源', () => {
  /*
   * ⚠️ issue #6 還有一條「Rich Menu 格子若設 FLEX_POPUP 也走同一組組裝函式」，
   * **本輪沒有落地**，所以這裡沒有對應的斷言，也沒有 `richMenuCellAction()`。
   * 前提還不成立：main 的 `POST /api/settings/line/rich-menu/create` 六格
   * action 是寫死的 `CELL_TEXTS`，沒有任何「每格自訂」的儲存後端（屬 #7 / #19）。
   * 先搬一支沒有呼叫端的函式過來，只會讓讀到的人以為每格設定已經會生效。
   * 那一段落地時，這裡要補回「FLEX_POPUP 的格子送出的文字解析得到 MENU」的斷言。
   */
...
describe('卡片上限 12 的單一事實來源（MAX_FLEX_CARDS）', () => {
  ...
  it('頁面用 MAX_FLEX_CARDS 擋，沒有自己寫死一個 12（頁面層）', () => {
    const page = readFileSync(resolve(ROOT, 'src/app/tenant/rich-menu-design/page.tsx'), 'utf8');
    // 比對「有 import 進來」而不是某一種排版：換行寫法不該讓這條鎖變紅，
    // 它要防的是頁面自己寫死一個 12，不是 import 的斷行位置。
    expect(page).toMatch(/import\s*\{[^}]*\bMAX_FLEX_CARDS\b[^}]*\}\s*from\s*'@\/config\/tenant-settings'/);
