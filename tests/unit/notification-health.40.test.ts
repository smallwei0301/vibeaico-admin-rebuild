...
import { dailyHealthReportPeriod } from '@/server/notifications/health-report';

describe('notification health digest (#40, 17 §6)', () => {
  it('uses the completed Asia/Taipei 09:00 bucket so delayed daily cron attempts keep one report and event key', () => {
    const scheduled = dailyHealthReportPeriod(new Date('2030-06-05T02:00:00.000Z'));
    const delayed = dailyHealthReportPeriod(new Date('2030-06-05T02:45:00.000Z'));
    expect(scheduled).toEqual({ bucket: '2030-06-05', periodStart: '2030-06-04T01:00:00.000Z', periodEnd: '2030-06-05T01:00:00.000Z' });
    expect(delayed).toEqual(scheduled);
  });
