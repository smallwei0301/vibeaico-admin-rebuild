...
function observe(prs: any[], workflowRunsBySha: Record<string, any[]> = {}, workflowUnavailableShas: string[] = []): any {
...
describe('governance current observation (#466)', () => {
  ...
  it('derives first-pass numerator/denominator across PRs', () => {
    ...
    const result = observe([a, b], {
      ...
      [b.head.sha]: [run('ci', 'failure', '2026-09-15T00:10:00Z', b.head.sha), run('ci', 'success', '2026-09-15T00:20:00Z', b.head.sha)],
    });
    expect(result.ci.firstPass).toEqual({ numerator: 1, denominator: 2, percent: 50 });
  });

  it('counts WIP failure→success on the same final head as recovery proxy', () => {
    const item = pr();
    const result = observe([item], {
      [item.head.sha]: [
        run('Agent WIP Policy', 'failure', '2026-09-15T00:10:00Z'),
        run('Agent WIP Policy', 'success', '2026-09-15T00:20:00Z'),
      ],
    });
    expect(result.ci.metadataGateRecoveryCount).toBe(1);
  });

  it('reports scope-budget violations and stale terminal lifecycle', () => {
    const item = pr({
      state: 'closed',
      merged_at: '2026-09-15T01:00:00Z',
      changed_files: 9,
  ...
  it('keeps workflow-derived metrics null when history is unavailable while preserving other facts', () => {
    const item = pr();
    const result = observe([item], {}, [item.head.sha]);
    expect(result.counts.totalGovernancePrs).toBe(1);
