...
describe('migration identity comparer', () => {
  ...
  it('rejects unsanitized, stale, cross-project, malformed, and duplicate evidence', () => {
    ...
    expect(() => normalizeMigrationIdentitySnapshot(snapshot({ observedAt: '2026-02-30T12:00:00Z' }), 'TEST', MAIN))
      .toThrow(/INVALID_OBSERVED_AT/);
    expect(() => normalizeMigrationIdentitySnapshot(snapshot({ observedAt: '2024-02-29T12:00:00Z' }), 'TEST', MAIN))
      .not.toThrow();
  ...
  it('writes a deterministic JSON report atomically and preserves earlier output on invalid input', () => {
    ...
    try {
      ...
      writeFileSync(join(repoRoot, '.gitignore'), 'supabase/migrations/9999_ignored_candidate.sql\n');
      ...
      writeFileSync(join(repoRoot, 'supabase', 'migrations', '9999_ignored_candidate.sql'), 'select 9999;\n');
      expect(execFileSync('git', ['status', '--porcelain=v1', '--untracked-files=all'], { cwd: repoRoot, encoding: 'utf8' })).toBe('');
      execFileSync(process.execPath, [script, 'compare',
        '--ledger-snapshot', snapshotPath,
        '--repo-root', repoRoot,
        '--current-main-sha', head,
      ...
      const ignoredReport = JSON.parse(readFileSync(outputPath, 'utf8'));
      expect(ignoredReport.entries).toEqual([{
        prefix: '0001',
        ledgerIdentity: '0001_initial_schema',
        mainIdentity: '0001_initial_schema',
        status: 'ON_MAIN_EXACT',
