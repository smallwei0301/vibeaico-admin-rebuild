...
describe('notification transports (#40, 17 §3–4)', () => {
  it('records Resend API success as ACCEPTED, never DELIVERED', async () => {
    const sender = vi.fn(async () => ({ data: { id: 're_123' }, error: null }));
    const outcome = await sendEmailTransport(
      { apiKey: 'test-key', from: 'VibeAI <noreply@example.com>', to: 'owner@example.com', subject: '通知', html: '<p>ok</p>', idempotencyKey: 'delivery-123' },
      sender,
    );
    expect(outcome).toEqual({ kind: 'accepted', providerMessageId: 're_123' });
    expect(sender).toHaveBeenCalledWith(expect.objectContaining({ idempotencyKey: 'delivery-123' }));
  });

  it('does not call an Email provider when the platform transport is unconfigured', async () => {
    const sender = vi.fn();
    await expect(sendEmailTransport(
      { apiKey: '', from: 'VibeAI <noreply@example.com>', to: 'owner@example.com', subject: '通知', html: '<p>ok</p>', idempotencyKey: 'delivery-123' }, sender,
