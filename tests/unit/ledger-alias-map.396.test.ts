...
describe('#396 已提交的正式資料', () => {
  ...
  // repo 端多出尚未套用的 0105、0109 與本候選的 0110，正式庫快照維持 57 筆
  // 實際 ledger row（0106／0108／0107 已分別於 2026-09-14 經 Owner 具名授權
  // 套用並重新擷取本快照，三者都在快照裡）。
  // 數字不是推算的，是對 current main／submitted candidate 的實際檔案跑一次得到的。
  it('repo 有 59 個 migration 檔案，正式庫快照有 57 筆 ledger row', () => {
    expect(repoFiles).toHaveLength(59);
    expect(snapshot.ledgerRowNames).toHaveLength(57);
  });

  it('supabase/ledger-alias-map.json 完全涵蓋這 59 個 repo 檔案與 57 筆 ledger row', () => {
    const result = verifyLedgerAliasMap({
      repoFiles,
  ...
  it('分類統計符合已查證的事實：50 EXACT、6 ALIAS、3 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 再次重新擷取本快照（49 → 50）。尚未套用的是 0105、0109 與本候選的 0110，
    // 所以 NOT_APPLIED 是 3。三者依 AGENTS.md 的規則，在合併進 main 之前都不是
    // 任何環境的套用授權。
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 目前為 3 筆（0105、0109 與本候選 0110，皆 PENDING_APPLY；0107／0108 已套用
    // 正式庫轉為 EXACT）。保留 main 那一版的意圖：釘住數量而不是只檢查「每一筆
    // 都有理由」，否則清單變空時這條規則會靜悄悄變成空轉。任何人日後新增或移除
