...
describe('#396 已提交的正式資料', () => {
  ...
  // 實際檔案為準。
  it('repo 有 71 個 migration 檔案，正式庫快照有 58 筆 ledger row', () => {
    expect(repoFiles).toHaveLength(71);
    expect(snapshot.ledgerRowNames).toHaveLength(58);
  });

  it('supabase/ledger-alias-map.json 完全涵蓋這 71 個 repo 檔案與 58 筆 ledger row', () => {
    const result = verifyLedgerAliasMap({
      repoFiles,
  ...
  it('分類統計符合已查證的事實：51 EXACT、6 ALIAS、14 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 拆出的 migration-only successor），所以 NOT_APPLIED 是 14。
    ...
    expect(counts.NOT_APPLIED ?? 0).toBe(14);
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    expect(notApplied.length).toBe(14);
