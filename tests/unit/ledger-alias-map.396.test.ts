...
describe('#396 已提交的正式資料', () => {
  ...
  // repo 端尚未套用的是 0109、0110 與本候選的 0111；0105 已於 2026-09-15 出現在正式庫 provider ledger，正式庫快照為 58 筆
  ...
  it('分類統計符合已查證的事實：51 EXACT、6 ALIAS、3 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 再次重新擷取本快照（49 → 50）。2026-09-15 唯讀重擷取確認 0105 已在 Production
    // provider ledger，因此轉為 EXACT（50 → 51）。尚未套用的是 0109、0110 與本候選的
    // 0111，所以 NOT_APPLIED 是 3。
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 目前為 4 筆（0105、0109、0110 與本候選 0111，皆 PENDING_APPLY；0107／0108
