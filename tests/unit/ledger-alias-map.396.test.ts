...
describe('#396 已提交的正式資料', () => {
  ...
  // 0127_issue_589_authz_constraint_reconciliation、0128_issue_42_plan_seasonal_pricing
  // （已於本候選之前併入 main）、0129_issue_15_chat_images_bucket（issue #15：只
  // insert 一列 storage.buckets，補上 0126 政策白名單早就提到、卻從未真的建立的
  // chat-images bucket；原檔名 0128 與 main 既有的 0128_issue_42_plan_seasonal_pricing
  // 撞號，解衝突時改為 0129）；正式庫快照維持 58 筆實際 ledger row（0106／0108／0107
  ...
  it('分類統計符合已查證的事實：51 EXACT、6 ALIAS、21 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 撞號而改為 0129），加上 0131_issue_37_atomic_departure_staff
    // （#37 真缺口 A：trip_departure_staff 的 assignment persistence 原子性，
    // 只新增 service_role-only 的 replace_trip_departure_staff RPC，不接線任何
    // 呼叫端），加上本 PR 的 0132_issue_42_seasonal_price_resolution
    // （#42 真缺口：create_tour_order 補上季節定價解析，之前無條件用
    // price_per_person，季節 override 從未對任何成交訂單生效），所以
    // NOT_APPLIED 是 22。這 22 支依 AGENTS.md 的規則，在合併進 main 之前都不是
    // 任何環境的套用授權。
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 目前為 19 筆：0110、0111、0113、0115、0116_issue_18_owner_notify、0124_issue_18_owner_notify_legacy_shape、
    // 0125_issue_17_booking_addons_legacy_enum、0126_issue_402_keyword_reply_images_authz、
    // 0117_issue_25b_support_chat_threads、0118_issue_25c_platform_donations、
    // 0119_issue_18_owner_notify_confirm_atomic、0121_issue_17_booking_addons_hardening、
    // 0123_issue_589_richmenu_asset_retirement、0127_issue_589_authz_constraint_reconciliation、
    // 0128_issue_42_plan_seasonal_pricing 這 14 支是 PENDING_APPLY；0109（SCHEMA_REPAIR）、
    // 0112 與 0114（AUTHZ+BACKFILL 混合）、0129_issue_15_chat_images_bucket（BACKFILL，與本輪
