...
describe('#396 已提交的正式資料', () => {
  ...
  // main 側 PR，見 supabase/ledger-alias-map.json 對應 evidence）與本候選
  // 0119_issue_18_owner_notify_confirm_atomic（#18 缺口 A 併發修復，不改既有
  // 表結構／RLS，只新增兩支 service-role-only 函式），正式庫快照維持 58 筆
  ...
  it('分類統計符合已查證的事實：51 EXACT、6 ALIAS、11 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 與本候選的 0119_issue_18_owner_notify_confirm_atomic（後幾筆合併自
    // origin/main 的 #519/#524/#526/#527 等 PR），所以 NOT_APPLIED 是 11。
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 0117_issue_25b_support_chat_threads、0118_issue_25c_platform_donations、
    // 0119_issue_18_owner_notify_confirm_atomic 這 8 支是 PENDING_APPLY；
    // 0109（SCHEMA_REPAIR）、0112 與 0114（AUTHZ+BACKFILL 混合）是
