...
describe('#396 已提交的正式資料', () => {
  ...
  // repo 端多出尚未套用的 0105、0107 與本候選的 0108，正式庫快照維持 55 筆實際
  // ledger row（0106 已於 2026-09-14 經 Owner 具名授權套用，因此它在快照裡）。
  // 數字不是推算的，是對 current main／submitted candidate 的實際檔案跑一次得到的。
  it('repo 有 57 個 migration 檔案，正式庫快照有 55 筆 ledger row', () => {
    expect(repoFiles).toHaveLength(57);
    expect(snapshot.ledgerRowNames).toHaveLength(55);
  });

  it('supabase/ledger-alias-map.json 完全涵蓋這 56 個 repo 檔案與 55 筆 ledger row', () => {
    const result = verifyLedgerAliasMap({
      repoFiles,
  ...
  it('分類統計符合已查證的事實：48 EXACT、6 ALIAS、2 NOT_APPLIED、1 LEDGER_ONLY', () => {
    const counts: Record<string, number> = {};
    for (const entry of aliasMap.entries) {
      counts[entry.classification] = (counts[entry.classification] ?? 0) + 1;
    }
    // 2026-09-13：0069–0073 五支經 Owner 具名授權套用至正式庫並回讀驗證，
    // 因此從 NOT_APPLIED 轉為 EXACT（42 → 47）。0105、0107 已進 main 但尚未套用，
    // 0106 已套用至 Production 並完成 postflight。尚未套用的是 0105、0107 與本候選
    // 的 0108，所以 NOT_APPLIED 是 3。0105／0107／0108 依 AGENTS.md 的規則，在合併
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 目前為 3 筆（0105、0107 與本候選 0108，皆 PENDING_APPLY）。保留 main 那一版
    // 的意圖：釘住數量而不是只檢查「每一筆都有理由」，否則清單變空時這條規則會
