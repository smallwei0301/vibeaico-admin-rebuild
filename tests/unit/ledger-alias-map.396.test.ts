...
describe('#396 已提交的正式資料', () => {
  ...
  it('分類統計符合已查證的事實：51 EXACT、6 ALIAS、11 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 0117_issue_25b_support_chat_threads、0118_issue_25c_platform_donations 與
    // 本候選的 0119_issue_46_public_request_journey（後幾筆合併自
    // origin/main 的 #519/#524/#526/#527 等 PR），所以 NOT_APPLIED
    // 是 11。十一者依 AGENTS.md 的規則，在合併進 main 之前都不是任何環境的套用授權。
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 0119_issue_46_public_request_journey 這 8 支是 PENDING_APPLY；0109
    // （SCHEMA_REPAIR）、0112 與 0114（AUTHZ+BACKFILL 混合）是 VERIFIED_NOT_APPLIED。
    // 0107／0108 已套用正式庫轉為 EXACT；0105 於
