...
describe('#396 已提交的正式資料', () => {
  ...
  // repo 端多出尚未套用的 0105 與本候選的 0107，正式庫快照維持 55 筆實際 ledger row
  // （0106 已於 2026-09-14 經 Owner 具名授權套用，因此它在快照裡）。
  // 數字不是推算的，是對 current main／submitted candidate 的實際檔案跑一次得到的。
  it('repo 有 56 個 migration 檔案，正式庫快照有 55 筆 ledger row', () => {
    expect(repoFiles).toHaveLength(56);
  ...
  it('分類統計符合已查證的事實：48 EXACT、6 ALIAS、2 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 0106 已套用至 Production 並完成 postflight。尚未套用的是 0105 與本候選的 0107，
    // 所以 NOT_APPLIED 是 2。0107 依 AGENTS.md 的規則，在合併進 main 之前不是任何
    // 環境的套用授權。
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 目前為 2 筆（0105 與 0107，皆 PENDING_APPLY）。保留 main 那一版的意圖：
