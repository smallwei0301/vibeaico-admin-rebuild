...
describe('#396 已提交的正式資料', () => {
  ...
  // 0121_issue_17_booking_addons_hardening（依序合併自 #519/#524/#526/#527 等
  // main 側 PR 加上本 issue #17 拆出的 migration-only PR，見
  // supabase/ledger-alias-map.json 對應 evidence），正式庫快照維持 58 筆
  ...
  it('分類統計符合已查證的事實：51 EXACT、6 ALIAS、10 NOT_APPLIED、1 LEDGER_ONLY', () => {
    ...
    // 與本候選的 0121_issue_17_booking_addons_hardening
    // （後幾筆合併自 origin/main 的 #519/#524/#526/#527 等 PR 加上本 issue #17
    // 拆出的 migration-only PR），所以 NOT_APPLIED 是 11。十一者依 AGENTS.md
    // 的規則，在合併進 main 之前都不是任何環境的套用授權。0109 是
    // SCHEMA_REPAIR；0112、0114 是 AUTHZ+BACKFILL 混合風險。三者各自留在既有
    // migration 歷史中，並標為 VERIFIED_NOT_APPLIED，直到未來獨立 release
    // 有對應的審查與執行器。
...
describe('#396 NOT_APPLIED 的兩種狀態必須用列舉講清楚', () => {
  ...
  it('已提交的正式對照表：每一筆 NOT_APPLIED 都有合法的 notAppliedReason', () => {
    ...
    // 0117_issue_25b_support_chat_threads、0118_issue_25c_platform_donations、
    // 0121_issue_17_booking_addons_hardening 這 8 支是 PENDING_APPLY；
