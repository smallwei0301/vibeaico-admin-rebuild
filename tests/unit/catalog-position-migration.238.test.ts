...
const migration = readFileSync(
  fileURLToPath(new URL('../../supabase/migrations/0096_catalog_position_invariants.sql', import.meta.url)),
  'utf8',
);
...
/** Mirrors the four ranking phases in 0096, so the direction assertions below have a behavior oracle. */
function apply0096Ranking(input: Row[]): Row[] {
...
describe('#238 0096 migration preserves catalog order through negative staging', () => {
  ...
  it('the four-phase ranking preserves both lanes and a second run is idempotent', () => {
    ...
    const once = apply0096Ranking(original);
    ...
    const twice = apply0096Ranking(once);
