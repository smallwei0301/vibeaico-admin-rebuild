...
describe('#589 authz and notified constraint reconciliation candidate', () => {
  it('fails closed on unknown policies and on legacy PENDING data before changing the check', () => {
    expect(migration).toContain('A single procedural statement keeps');
    expect(migration).toMatch(/^do \$reconcile\$\s*declare[\s\S]*?begin/m);
    expect(migration).not.toMatch(/^\s*begin;$/im);
    ...
    expect(migration.indexOf('lock table public.booking_addons')).toBeLessThan(migration.indexOf('alter table public.booking_addons enable'));
    expect((migration.match(/^do\s+\$reconcile\$/gim) ?? [])).toHaveLength(1);
    expect(migration).not.toMatch(/\b(?:commit|rollback)\b|\bexception\s+when\b/i);
    ...
    expect(migration).toContain("pg_catalog.aclexplode(coalesce(c.relacl, pg_catalog.acldefault('r', c.relowner)))");
  ...
  it('removes only legacy public CRUD policies and states the intended grants explicitly', () => {
    ...
    expect(migration).toContain('create policy p_booking_addons_s on public.booking_addons for select to authenticated');
    ...
    expect(migration).toContain('create policy p_owner_notify_recipients_all on public.owner_notify_recipients for all to authenticated');
