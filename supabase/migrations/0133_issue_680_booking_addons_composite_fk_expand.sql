...
BEGIN
  ...
  LOOP
    ...
    IF fk.conname = 'booking_addons_tenant_id_performance_staff_id_fkey' AND NOT (
      ...
      AND NOT fk.condeferred
      AND (
        (fk.confdeltype = 'a' AND fk.confdelsetcols IS NULL)
        OR (fk.confdeltype = 'n' AND fk.confdelsetcols = ARRAY[child_perf]::smallint[])
      )
    ) THEN
  ...
  -- EXPAND preserves either known composite delete action. The single-column
  -- compatibility FK clears performance_staff_id in both variants; a later CONTRACT decision
  -- may choose one composite action after Product runtime acceptance.
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint c
     WHERE c.conrelid = child_oid
     ...
     OR NOT EXISTS (
       SELECT 1 FROM pg_constraint c
        WHERE c.conrelid = child_oid
          ...
          AND c.convalidated AND NOT c.condeferrable AND NOT c.condeferred
          AND (
            (c.confdeltype = 'a' AND c.confdelsetcols IS NULL)
            OR (c.confdeltype = 'n' AND c.confdelsetcols = ARRAY[child_perf]::smallint[])
          )
     ) THEN
