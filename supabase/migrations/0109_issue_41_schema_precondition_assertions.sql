...
--
-- 這個「依賴前提卻沒斷言它」的病，在同一天於三個不同層次各發作一次：`0108` 的
-- 後置斷言（沒斷言 → 靜默通過）、上面這個第四段第一版（沒斷言 → migration 中止）、
-- 以及已合併上線的 `/api/guide/action-inbox`（沒有任何檢查 → 正式環境 500，因為它
-- 同樣讀取 `trip_departures.formation_status`，而正式庫當時還沒套用 0107）。第三個
-- 層次的完整事故記錄見 `docs/schema-truth/2026-09-14-production-0107-not-applied.md`
-- ——那份文件記的教訓是「`PRODUCTION_SCHEMA_STATUS` 這一格填 `NOT_REQUIRED` 而不去
-- 查正式庫，等於沒有這一格」，而本檔存在的目的正是讓這類前提變成機器可查、可斷言
-- 的東西，不是留給人工填一個沒人驗證的欄位。Owner 已具名授權把 0107 套用到正式庫
-- （provider ledger row `20260914094736`），第三個層次的**這一起事故**已排除；但
-- 第四段的存在性守衛仍然保留，因為它守的是「0107 尚未套用」這個環境形狀本身，
-- 不是這一起事故——任何新建的、只套用到一半的環境都可能重新落入這個形狀。
--
