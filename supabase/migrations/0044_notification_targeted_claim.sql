-- 0044 — targeted claim for post-commit notification events
--
-- Generic workers remain bounded by claim/materialization limits. A just-created
-- post-commit event (notably a cron reminder) must not wait behind an older
-- OPEN backlog, so the caller can atomically claim only its own delivery row.
-- This is service-side only and does not expose a browser RPC.
