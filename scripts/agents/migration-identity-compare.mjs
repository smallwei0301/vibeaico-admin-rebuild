...
export function normalizeMigrationIdentitySnapshot(value, expectedEnvironment, currentMainSha) {
  ...
  if (typeof value.observedAt !== 'string' || !isValidIsoUtc(value.observedAt)) {
...
function readMainMigrationManifest(repoRoot, currentMainSha) {
  const repository = fs.realpathSync(path.resolve(repoRoot));
  const result = spawnSync('git', [
    '-C', repository,
    'ls-tree', '-r', '--full-tree', currentMainSha, '--', 'supabase/migrations',
  ], { encoding: 'utf8' });
  if (result.status !== 0) fail('GIT_TREE_UNAVAILABLE', result.stderr?.trim() || 'main migration tree could not be read');

  const files = [];
  for (const line of result.stdout.split('\n').filter(Boolean)) {
    const match = /^(\d{6}) (\w+) [0-9a-f]{40}\t(.+)$/.exec(line);
    if (!match) fail('INVALID_GIT_TREE_ENTRY', `unable to parse migration tree entry: ${line}`);
    const [, mode, type, filePath] = match;
    if (mode === '120000') fail('MIGRATION_SYMLINK_REJECTED', `migration tree contains a symlink: ${filePath}`);
    if (type !== 'blob') fail('INVALID_MAIN_MIGRATION_IDENTITY', `migration tree entry is not a file: ${filePath}`);
    if (filePath.endsWith('.sql')) files.push({ path: filePath });
  }
  return { files };
}
...
function verifyExactRepositoryHead(repoRoot, currentMainSha) {
  ...
  const status = spawnSync('git', ['-C', repository, 'status', '--porcelain=v1', '--untracked-files=all'], { encoding: 'utf8' });
  if (status.status !== 0) fail('GIT_STATUS_UNAVAILABLE', 'repoRoot status could not be read');
  if (status.stdout.trim()) fail('DIRTY_REPOSITORY', `repoRoot has uncommitted files: ${status.stdout.trim()}`);
...
export function compareMigrationIdentitySnapshot({ snapshot, migrationManifest, currentMainSha }) {
  ...
  const entries = normalized.ledgerState === 'PRESENT' ? prefixes.flatMap((prefix) => {
    const ledgerIdentities = ledgerByPrefix.get(prefix) ?? [];
    const mainIdentity = mainByPrefix.get(prefix) ?? null;
    if (ledgerIdentities.length === 0) {
...
export function compareMigrationIdentityAtRepo({ snapshot, repoRoot, currentMainSha }) {
  const mainSha = normalizeCurrentMainSha(currentMainSha);
  const before = verifyExactRepositoryHead(repoRoot, mainSha);
  const report = compareMigrationIdentitySnapshot({
    snapshot,
    migrationManifest: readMainMigrationManifest(repoRoot, mainSha),
    currentMainSha: mainSha,
  });
  const after = verifyExactRepositoryHead(repoRoot, mainSha);
  if (before.head !== after.head) fail('STALE_REPOSITORY_HEAD', 'repo HEAD changed while the migration manifest was read');
  return report;
}
