...
const PRIVILEGES = new Set([
  'SELECT', 'INSERT', 'UPDATE', 'DELETE', 'TRUNCATE', 'REFERENCES', 'TRIGGER', 'EXECUTE',
]);
...
WITH items AS (
  ...
  UNION ALL
  SELECT 'views', schemaname || '.' || viewname, definition
  FROM pg_views
  ...
  SELECT 'policies', n.nspname || '.' || c.relname || '.' || p.polname,
    p.polcmd::text || '|' ||
      COALESCE(array_to_string(ARRAY(
        SELECT r.rolname FROM pg_roles AS r WHERE r.oid = ANY(p.polroles)
      ), ','), '') || '|' ||
...
SELECT json_build_object(
  'tables', (SELECT json_agg(json_build_object(
      'schema', table_schema, 'name', table_name, 'grantee', grantee,
      'privilege', privilege_type, 'grantable', is_grantable
    ) ORDER BY table_schema, table_name, grantee, privilege_type)
    FROM information_schema.role_table_grants
    WHERE table_schema = 'public'),
  'tableSecurity', (SELECT json_agg(json_build_object(
